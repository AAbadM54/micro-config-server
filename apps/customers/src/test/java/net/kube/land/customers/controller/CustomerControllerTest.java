...
public class CustomerControllerTest {
        ...
        this.mockMvc.perform(get("/kube/customers").accept(MediaType.APPLICATION_JSON))
                ...
                .andDo(document("list-customers", preprocessRequest(prettyPrint()),
                        preprocessResponse(prettyPrint()), responseFields(
                                ...
                                fieldWithPath("[].customerSchool").type(JsonFieldType.STRING).optional().description("Customer School"),
                                fieldWithPath("[].customerProfession").type(JsonFieldType.STRING).optional().description("Customer Profession"),
                                fieldWithPath("[].customerFriends").type(JsonFieldType.STRING).optional().description("Customer Friends"),
                                fieldWithPath("[].customerEnemy").type(JsonFieldType.STRING).optional().description("Customer Enemy")
                        )
                )
        );
    }
    ...
    public void getCustomer() throws Exception {
        this.mockMvc.perform(get("/kube/customers/100").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
